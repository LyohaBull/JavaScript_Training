...
let progressState = {
	...
	getState(){
		...
		if (algorithm.value == "MD5" || algorithm.value == "SHA256" || algorithm.value == "SHA512") {
			this["50%"]=true;
			return;
		}
		if (this.isFileKey) {
...
class Encryption{
	...
	crypto(){
		...
		do {
			...
			this.i++;
		}while (this.i<k+this.step)
		if (this.i < this.opentext.length) {
			Bar.innerHTML = +((100*k/this.length).toFixed(2)) +"%";
			Bar.style.background = `linear-gradient(to right, #37B95E ${+((100*k/this.length).toFixed(2))}%, transparent ${+((100*k/this.length).toFixed(2))}%)`;
	...
	new Promise((resolve)=>{
		if (progressState.isFile) {
			...
			return new Promise((resolve)=>{
				...
				else resolve(keys_bin.value.split(" ").map(item=>parseInt(item,16)));
			})
		
	}).then((keyR)=>{
				encryption = new Encryption(tt,keyR);
				encryption.start();
